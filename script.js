...
if (touchZone) {
    ...
    const handleMining = (e) => {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            // --- ВОТ ЭТИ ДВЕ СТРОЧКИ ИСПРАВЛЯЮТ X2 ---
            const alphaMult = window.userMultiplier || 1; // Берем 2.0 если Альфа, иначе 1
...
window.claimAlphaStatus = function() {
    ...
    // 1. Быстрая проверка локально (если класс уже есть)
    if (btn && btn.classList.contains('activated')) return;

    // 2. Проверка в базе данных (чтобы нельзя было нажать дважды)
    db.ref('users/' + user.id + '/isAlphaNode').once('value', (snap) => {
        if (snap.val() === true) {
            tg.showAlert("Вы уже являетесь ALPHA-участником!");
            if (btn) btn.classList.add('activated');
            return;
        }

        // 3. Если еще не участник, запускаем процесс
        tg.showConfirm("Стать ALPHA-тестером бесплатно? (x2 доход)", (ok) => {
            if (ok) {
                db.ref('global/alphaNodesCount').transaction((count) => {
                    if ((count || 0) < 1000) return (count || 0) + 1;
                    return; 
                }, (error, committed) => {
                    if (committed) {
                        // Записываем статус игроку
                        db.ref('users/' + user.id).update({ 
                            isAlphaNode: true,
                            miningMultiplier: 2.0,
                            betaTester: true 
                        }).then(() => {
                            window.userMultiplier = 2.0;
                            if (btn) btn.classList.add('activated');
                            tg.HapticFeedback.notificationOccurred('success');
                            tg.showAlert("УСПЕХ! Теперь вы в списке 1000 первых.");
                        });
                    } else {
                        tg.showAlert("Увы, места закончились!");
                    }
                });
            }
        });
        ...
        // Запускаем проверку счетчика и личного статуса при старте
        if (typeof db !== 'undefined' && user?.id) {
            ...
            db.ref('global/alphaNodesCount').on('value', (snapshot) => {
                ...
                if (count >= 1000) {
                    const btn = document.getElementById('alpha-node-btn');
                    // Активируем класс (серый цвет), только если юзер сам не является Альфой
                    if (btn && !window.userMultiplier) btn.classList.add('activated');
                }
            });

            // Проверка личного статуса игрока
            db.ref('users/' + user.id + '/isAlphaNode').once('value', (snap) => {
                if (snap.val() === true) {
                    window.userMultiplier = 2.0;
                    const btn = document.getElementById('alpha-node-btn');
