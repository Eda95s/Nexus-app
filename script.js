...
function checkPromo() {
    ...
    // Тот самый код, который ты настроил в PuzzleBot
    if (val === 'NEXUS-BOOST-2026') {
        // Проверка, чтобы не активировали дважды
        if (localStorage.getItem('promo_2026_used')) {
        ...
        // 2. СОХРАНЯЕМ В ПАМЯТЬ (чтобы баланс не обнулялся)
        localStorage.setItem('nexus_balance', balance);
        localStorage.setItem('promo_2026_used', 'true');

        // 3. Обновляем интерфейс
        if (typeof updateUI === "function") updateUI();
        
        tg.showAlert("✅ Успешно! Вам начислено 1,000,000 N");
